...
import { 
  ...
  Camera, Video, Smartphone, Menu, Phone
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const tabLabels: Record<string, string> = {
    ...
    home_cms: 'CMS Halaman Home',
    route_cms: 'CMS Profil & Jalur',
    catalog_cms: 'CMS Katalog',
    articles: 'CMS Berita',
    galleries: 'CMS Galeri',
    routes: 'Status & Tarif SIMAKSI',
  ...
  const navContent = (isMobile: boolean) => (
    <div className="flex flex-col h-full">
      {/* Top Header */}
      <div className="p-5 border-b border-[#26261f] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white p-1 flex items-center justify-center shadow-sm overflow-hidden shrink-0">
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col lg:flex-row text-[#050505]">
      ...
      {/* ===== 1. MOBILE TOP STICKY HEADER (VISIBLE ON MOBILE ONLY) ===== */}
      <header className="lg:hidden sticky top-0 z-30 bg-[#141410] text-white border-b border-[#26261f] px-4 py-3 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-2.5">
          <button
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            className="p-2 -ml-1 rounded-xl bg-white/10 hover:bg-white/20 text-white cursor-pointer"
            aria-label="Toggle Menu"
          >
            {isMobileMenuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </button>
          <div className="w-8 h-8 rounded-lg bg-white p-0.5 flex items-center justify-center overflow-hidden shrink-0">
            <img src={logoBc} alt="Logo BC" className="w-full h-full object-contain" />
          </div>
          <div>
            <span className="text-[10px] text-amber-300 font-bold block leading-none">Admin Basecamp</span>
            <span className="text-xs font-black text-white truncate max-w-[140px] block leading-tight">
              {tabLabels[activeTab] || 'Console Admin'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-2 rounded-xl bg-[#0D5C3A] text-white hover:bg-[#064e3b] transition-all flex items-center gap-1 text-[11px] font-black shadow-xs cursor-pointer"
            title="Scan QR Tiket"
          >
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          />
          {/* Drawer Content */}
          <aside className="relative w-4/5 max-w-xs bg-[#141410] text-white z-50 flex flex-col shadow-2xl h-full">
            {navContent(true)}
          </aside>
        </div>
      )}
      ...
      <aside className="hidden lg:flex w-72 bg-[#141410] text-white flex-shrink-0 flex-col border-r border-[#26261f]">
        {navContent(false)}
      </aside>

      {/* ===== 4. MOBILE BOTTOM QUICK ACTION DOCK (STICKY FOOTER BAR) ===== */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-[#141410]/95 backdrop-blur-md border-t border-[#26261f] px-2 py-1.5 flex items-center justify-around text-[10px] font-bold shadow-2xl">
        <button
          onClick={() => setActiveTab('overview')}
          className={`flex flex-col items-center gap-1 py-1 px-2 rounded-xl transition-all cursor-pointer ${
            activeTab === 'overview' ? 'text-amber-300' : 'text-slate-400 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 py-1 px-2 rounded-xl transition-all cursor-pointer ${
            activeTab === 'bookings' ? 'text-amber-300' : 'text-slate-400 hover:text-white'
          }`}
        >
        ...
        {/* Center Prominent QR Scan Button */}
        <button
          onClick={() => {
            setScanCodeInput('');
            setScannedBookingResult(null);
          ...
          className="-mt-5 w-12 h-12 rounded-full bg-gradient-to-tr from-[#064e3b] to-[#0D5C3A] text-white flex items-center justify-center shadow-lg shadow-emerald-950/50 border-2 border-amber-300 cursor-pointer transition-transform active:scale-95"
          title="Scan QR Tiket"
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 py-1 px-2 rounded-xl transition-all cursor-pointer ${
            activeTab === 'route_cms' ? 'text-amber-300' : 'text-slate-400 hover:text-white'
          }`}
        >
          <Mountain className="w-4 h-4" />
          <span>Jalur & Profil</span>
        </button>

        <button
          onClick={() => setIsMobileMenuOpen(true)}
          className="flex flex-col items-center gap-1 py-1 px-2 rounded-xl text-slate-400 hover:text-white transition-all cursor-pointer"
        >
      ...
      {/* ===== 5. MAIN CONTENT PANE ===== */}
      <main className="flex-1 p-4 sm:p-6 lg:p-10 pb-24 lg:pb-10 overflow-y-auto max-w-7xl">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-[#F4F0E8] rounded-3xl border border-[#e7e5e4] overflow-hidden shadow-xs">
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-[#e7e5e4]">
                    ...
                    {filteredBookings.length === 0 && (
                      <tr>
                        <td colSpan={8} className="p-8 text-center text-xs text-[#707070]">
              ...
              <div className="md:hidden divide-y divide-[#e7e5e4]">
                {filteredBookings.map((b: any) => (
                  <div key={b.id} className="p-4 space-y-3 hover:bg-[#FAF8F5]/50 transition-colors">
                    <div className="flex items-center justify-between">
                      <span className="font-mono font-black text-xs text-[#050505] bg-[#FAF8F5] px-2.5 py-1 rounded-lg border border-[#e7e5e4]">
                        {b.kode_booking}
                      </span>
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase ${
                        b.status === 'CheckedIn'
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="font-black text-sm text-[#050505]">{b.nama_ketua}</h4>
                        <div className="flex items-center gap-2 mt-0.5 text-xs text-[#707070]">
                          <span>{b.no_hp}</span>
                          {b.no_hp && (
                            <a
                              href={`https://wa.me/${b.no_hp.replace(/^0/, '62').replace(/\D/g, '')}`}
                              target="_blank"
                              rel="noreferrer"
                      ...
                      <div className="text-right">
                        <span className="text-[10px] text-[#707070] block">Waktu Daftar:</span>
                        <span className="text-[11px] font-bold text-[#050505]">
                          {b.created_at ? new Date(b.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : '-'}
                        </span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2 p-2.5 bg-[#FAF8F5] rounded-xl text-xs">
                      <div>
                        <span className="text-[10px] text-[#707070] block">Jadwal Pendakian:</span>
                        <span className="font-bold text-[#050505]">
                          {new Date(b.tanggal_naik).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })} - {new Date(b.tanggal_turun).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                        </span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] text-[#707070] block">Peserta & Total:</span>
                        <span className="font-bold text-[#0D5C3A]">{b.jumlah_peserta} Org</span> &bull; <span className="font-black text-[#050505]">Rp {b.total_harga.toLocaleString('id-ID')}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      {(b.status === 'Pending' || b.status === 'Unpaid') && (
                        <button
                          onClick={() => bookingStatusMutation.mutate({ id: b.id, status: 'Paid' })}
                          className="flex-1 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-black text-center cursor-pointer shadow-xs transition-colors"
                        >
                          Konfirmasi Bayar
                        </button>
                      )}
                      {(b.status === 'Paid' || b.status === 'Confirmed') && (
                        <button
                          onClick={() => bookingStatusMutation.mutate({ id: b.id, status: 'CheckedIn' })}
                          className="flex-1 py-2 bg-[#0D5C3A] hover:bg-[#064e3b] text-white rounded-xl text-xs font-black text-center cursor-pointer shadow-xs transition-colors"
                        >
                          Check-In
                        </button>
                      )}
                      {b.status === 'CheckedIn' && (
                        <button
                          onClick={() => bookingStatusMutation.mutate({ id: b.id, status: 'CheckedOut' })}
                          className="flex-1 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-black text-center cursor-pointer shadow-xs transition-colors"
                        >
                          Check-Out
                        </button>
                      )}
                      <button
                        onClick={() => setSelectedBookingModal(b)}
                        className="px-4 py-2 bg-white border border-[#e7e5e4] text-[#050505] rounded-xl text-xs font-bold hover:bg-slate-100 cursor-pointer shadow-2xs"
                      >
                        Detail
                      </button>
                    </div>
