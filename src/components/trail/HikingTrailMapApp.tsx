...
export default function HikingTrailMapApp() {
  ...
  return (
    <div className="w-full bg-white dark:bg-[#F4F0E8] border border-slate-200 dark:border-[#e7e5e4] rounded-3xl shadow-sm overflow-hidden p-4 sm:p-6 space-y-5">
      
      {/* Top Control Bar: PETA DASAR Toggles & GPX Download */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-[#e7e5e4]">
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">PETA DASAR</span>
          <div className="flex gap-0.5 border border-slate-200 dark:border-[#e7e5e4] rounded-xl overflow-hidden bg-slate-50 dark:bg-[#FAF8F5] p-0.5">
        ...
        <button
          ...
          className="bg-[#0D5C3A] hover:bg-[#09442a] text-white px-3.5 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer self-start sm:self-auto"
        >
      ...
      <div className="relative w-full h-[340px] sm:h-[400px] rounded-2xl overflow-hidden border border-slate-200 dark:border-[#e7e5e4] shadow-inner bg-slate-900">
        <div ref={mapContainerRef} className="w-full h-full z-0" />

        {/* Map Legend Overlay */}
        <div className="absolute bottom-3 left-3 z-[400] bg-white/90 dark:bg-[#0c0a09]/90 backdrop-blur-md text-slate-800 dark:text-white p-2.5 rounded-xl border border-slate-200 dark:border-stone-800 shadow-md max-w-[180px] sm:max-w-xs text-[10px] space-y-1.5">
          <div className="font-extrabold text-[9px] uppercase tracking-wider text-[#0D5C3A] dark:text-emerald-400 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-1">
            <span>Legend / Ikon Peta</span>
            <Info className="w-3 h-3" />
          </div>
          <div className="grid grid-cols-2 gap-x-2 gap-y-1 font-semibold text-slate-700 dark:text-slate-200">
            ...
            <div className="flex items-center gap-1"><span className="w-2.5 h-1 bg-[#ea580c] rounded-full"></span> <span>Jalur Pendakian</span></div>
          </div>
        </div>
      </div>

      {/* Section: Profil Elevasi Chart */}
      <div className="pt-2 space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="font-display font-black text-sm text-slate-900 dark:text-[#050505]">Profil Elevasi</h3>
          <span className="text-[10px] font-semibold text-[#ea580c]">Geser kursor di grafik untuk melihat posisi</span>
        </div>

        <div className="w-full h-44 sm:h-52 bg-slate-50/50 dark:bg-[#FAF8F5] p-2 rounded-2xl border border-slate-100 dark:border-[#e7e5e4]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={CHART_DATA} margin={{ top: 10, right: 15, left: -15, bottom: 10 }}>
              <defs>
                <linearGradient id="elevationGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#0D5C3A" stopOpacity={0.25}/>
              ...
              <XAxis 
                ...
                label={{ value: 'Jarak (km)', position: 'insideBottom', offset: -8, fill: '#78716c', fontSize: 9, fontWeight: 700 }}
              />
              <YAxis 
                domain={[1400, 3400]} 
                unit="m" 
                ...
                tickLine={false}
                tick={{ fontSize: 9, fontWeight: 600 }}
                label={{ value: 'Elevasi (m)', angle: -90, position: 'insideLeft', offset: 12, fill: '#78716c', fontSize: 9, fontWeight: 700 }}
              />
              <Tooltip 
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-[#050505] text-white p-2 rounded-lg border-0 shadow-lg text-[11px] space-y-0.5">
                        <p className="font-extrabold text-[#ea580c]">{data.name}</p>
                        <p className="text-slate-300">Elevasi: <span className="font-bold text-white">{data.elevation} m</span></p>
              ...
              {CHECKPOINTS.map((cp) => (
                <ReferenceDot 
                  ...
                  y={cp.elevation} 
                  r={4} 
                  fill={cp.type === 'summit' ? '#1E293B' : cp.type === 'basecamp' ? '#0D5C3A' : '#EA580C'} 
                  stroke="#FFFFFF" 
                  strokeWidth={1.5}
                />
              ))}
      ...
      <div className="pt-2 space-y-2">
        <h4 className="text-[10px] font-bold tracking-widest text-[#ea580c] uppercase">
          ...
          {CHECKPOINTS.map((cp) => {
            ...
            return (
              <div
                ...
                className={`cursor-pointer p-3 rounded-xl border transition-all duration-150 flex items-center justify-between ${
                  isSelected 
                    ? 'bg-white dark:bg-[#FAF8F5] border-[#0D5C3A] ring-1 ring-[#0D5C3A]/25 shadow-xs' 
                    : 'bg-white dark:bg-[#FAF8F5] border-slate-200/80 dark:border-[#e7e5e4] hover:border-[#0D5C3A] shadow-2xs'
                }`}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${dotBg}`}></span>
                  <div className="truncate">
                    <h5 className="text-[11px] font-bold text-slate-800 dark:text-[#050505] truncate">
                      {cp.name} <span className="text-slate-400 dark:text-slate-500 font-normal">({cp.elevation} m)</span>
                    </h5>
                  </div>
                </div>

                <span className="text-[10px] font-semibold text-slate-400 shrink-0 ml-1.5">
                  {cp.distance} km
                </span>
              </div>
            );
